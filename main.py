...
if args.mode == "screen":
    ...
    os.system("sudo -S modprobe -r v4l2loopback && sudo modprobe v4l2loopback devices=1 video_nr=3 card_label='Virtual' exclusive_caps=1 max_buffers=2")

    def show(img):
        global cam
        if cam is None:
            h, w, c = img.shape
            cam = pyvirtualcam.Camera(width=w, height=h, fps=20, device='/dev/video3')
        if len(img.shape) == 2:
